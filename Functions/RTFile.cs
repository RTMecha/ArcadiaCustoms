...
namespace ArcadiaCustoms.Functions
{
	public class RTFile : MonoBehaviour
	{
		...
		{
			ArcadePlugin.beatmapThemes.Clear();
			ArcadePlugin.beatmapThemesIDToIndex.Clear();
			ArcadePlugin.beatmapThemesIndexToID.Clear();
			int num = 0;
			foreach (DataManager.BeatmapTheme beatmapTheme in ArcadePlugin.beatmapThemes)
			{
				ArcadePlugin.beatmapThemesIDToIndex.Add(num, num);
				ArcadePlugin.beatmapThemesIndexToID.Add(num, num);
				num++;
			}
			if (ArcadePlugin.gameData.beatmapData == null)
			{
				ArcadePlugin.gameData.beatmapData = new DataManager.GameData.BeatmapData();
			}
			if (_themeData != null)
			{
		...
		public static bool IsValidBeatmapFile(string filename)
		{
			...
			if (filename == "")
			{
				text = RTFile.GetBeatmapURL(DataManager.inst.GetSettingString("Selected Beatmap"));
			}
			else
			{
				text = filename;
		...
		// Token: 0x060007E9 RID: 2025 RVA: 0x0004BD48 File Offset: 0x00049F48
		...
		public static bool IsValidAudioFile(string filename)
		{
			...
			if (filename == "")
			{
				text = RTFile.GetAudioURL(DataManager.inst.GetSettingString("Selected Beatmap"));
			}
			else
			{
				text = filename;
		...
		// Token: 0x060007EC RID: 2028 RVA: 0x0004BE34 File Offset: 0x0004A034
		public static bool IsValidSettingsFile(string filename)
		{
			...
			if (filename == "")
			{
				settingsURL = RTFile.GetSettingsURL("Settings");
			}
			else
			{
				settingsURL = RTFile.GetSettingsURL(filename);
			}
			if (!File.Exists(settingsURL))
			{
				result = false;
			}
			else if (settingsURL == null || settingsURL == "")
			{
				result = false;
			}
			return result;
